...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.formControler = this.formBuilder.group({
      _nom: ['', [Validators.required, Validators.maxLength(10)]],

      _prenom: ['', [Validators.required, Validators.maxLength(10)]],
  ...
  submit() {
    if (
      this.formControler
        .valid /*&& this._password?.value  == this._passwordControl?.value*/
    ) {
